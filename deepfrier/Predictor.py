...
class DeepFool:
    ...
    def run_deepfool(self, fasta_sequence):
        initial_prediction = self.predictor.predict_function(fasta_sequence)
        mutation_thresholds = []

        for _ in range(50):
            mutated_sequence = list(fasta_sequence)
            grad_cam_scores = self.predictor.run_grad_cam(fasta_sequence)
    ...
    def plot_mutation_thresholds(self, mutation_thresholds, output_path):
